...
Set ANALYST_DATABASE_URL + ANALYST_DATA_DIR BEFORE importing src.db (the engine is built at import time).
FakeModel drives the ReAct loop offline (no API key) — harness/patterns/react-agent.md.
"""
import os
import tempfile

os.environ["ANALYST_DATABASE_URL"] = "sqlite+aiosqlite:///./test_agent.db"
os.environ.setdefault("ANALYST_DATA_DIR", tempfile.mkdtemp(prefix="datachat_test_"))
