...
A web-based data analysis agent that lets users connect data sources, then ask natural language questions about them in persistent sessions. Under the hood, the agent speaks the **Model Context Protocol (MCP)**: each data source is wrapped by an in-process MCP **server** that exposes executable tools (e.g., "run SQL query"), and the LangGraph ReAct loop acts as an MCP **client**, discovering and invoking those tools iteratively to answer the user's question from the full data.

For v0.1 the only supported data source type is a CSV file. Uploading a CSV converts it to Parquet and creates a `DataSource`; at query time the agent builds an MCP server that wraps that Parquet and exposes a `run_query` tool (backed by DuckDB). Future data source types (REST API, GraphQL, local shell commands) slot in by adding new MCP servers — no changes to the agent loop or UI shell.
...
- [x] Tools are exposed by a per-source MCP server and discovered by the agent at runtime via the MCP client — not hardcoded
...
- Tool input schemas are auto-generated by the MCP server (FastMCP) from the tool signature — not hand-written or stored
...
| 3 | MCP tool layer (per-source MCP servers over DuckDB/Parquet) + modular data sources | 🔄 In Progress |
