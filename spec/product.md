...
> `spec/capabilities/`; the `/analyze` preflight fails the build if any criterion has no capability.
...
(→ <slug>); the /analyze preflight enforces that every criterion has at least one. The P1 capability's
